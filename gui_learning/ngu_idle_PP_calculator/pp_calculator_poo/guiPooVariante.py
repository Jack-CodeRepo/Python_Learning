...
def timeOnePP(p, r):
    t = r + 0.8
    time_one_pp = 1000000/p*t

    return time_one_pp


def nbPPOverTime(onePP, tgtTime):
    # calcul du temps pour 1 PP
    pp_per_min = 60/onePP
    pp_per_hour = 60*pp_per_min
    result = ceil(pp_per_hour*tgtTime)
    pp_earned = result

    return pp_earned


def timeOverPP(goalPP, t):
    time_01_pp = ceil(t)
    time_goal = ceil(goalPP * t)
    # génération de valeur temporelles, attribution en variable
    time_un_pp = timedelta(seconds=time_01_pp)
    time_goal = timedelta(seconds=time_goal)

    return time_un_pp, time_goal
...
class interface(tk.Frame):
    ...
    def calculer_PP(self):
        ...
        tgtTime = self.saisie_goal_time.test_int(self.saisie_goal_time.get_value())
        tOnePP = timeOnePP(p, r)
        result = nbPPOverTime(tOnePP, tgtTime)
        # a chaque calcul, deux lignes de message sont générées et affichées
        string01 = f"{result} PP gagné en {tgtTime} heures."
    ...
    def calculer_temps(self):
        ...
        tOnePP = ceil(timeOnePP(p, r))
        tGoalPP = ceil(g * tOnePP)
        # génération de valeur temporelles, attribution en variable
        t01PP = timedelta(seconds=tOnePP)
        tNPP = timedelta(seconds=tGoalPP)
        # a chaque calcul, deux lignes de message sont générées et affichées
        string01 = f"Temps passé pour avoir 1 PP: {t01PP} hh:mm:ss"
        string02 = f"Temps passé pour avoir {g} PP: {tNPP} hh:mm:ss"
        self.display( string01 + '\n' + string02 )
